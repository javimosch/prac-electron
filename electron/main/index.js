...
const nanoid = async (len) => shortid.generate();
//quire("amd-loader");
...
function getCurrCfg(configurationName = "default") {
    if (!scope.currCfg) {
...
ipcMain.handle("getConfiguration", async (event, configurationName = "default.json") => {
...
ipcMain.handle("customAction", async (event, options = {}) => {
    ...
    if (options.name === "setConfigValues") {
        ...
        options.values.forEach((valueItem) => {
 ...
 * @returns
 */
function getStatItem(obj, attr = "sourceStats") {
    ...
    let statItem = scope.analysisStats[attr].find((item) => item.ext == extension);
    if (!statItem) {
        statItem = {
            ext: extension,
            count: 0,
...
function isFilteredFile(file, include) {
    return ((!file.isDirectory && include.length === 0) ||
        include.some((str) => str.toLowerCase() == file.extension.toLowerCase()));
}
ipcMain.handle("analyzeSources", async (event, sources = [], options = {}) => {
    ...
    options.include = (options.include || []).map((ext) => ext.toLowerCase());
    ...
    options.include.forEach((ext) => {
        options.include.push(ext.toUpperCase());
    });
    let extensions = options.include.map((ext) => (ext.charAt(0) === "." ? ext.substring(1) : ext).toLowerCase());
    extensions = extensions.filter((ext, i) => extensions.findIndex((e) => e == ext) == i);
    currCfg.set("extensions", extensions);
    consoleLog.debug("analyzeSources", JSON.stringify({
        sources,
        options,
    }, null, 4));
    ...
    let existingFilesInTargetDir = currCfg.get("existingFilesInTargetDir", []);
    ...
    if (!isAnalysisComplete) {
        ...
        sourceFiles = [];
        await sequential(sources.map((sourcePath) => {
            return async () => {
                //files
                let files = await rraListWrapper(sourcePath, {
                    mode: rra.LIST,
                    recursive: true,
                    stats: true,
                    ignoreFolders: true,
        ...
        await sequential(sourceFiles.map((file, index) => {
            return async () => {
                //grab md5
                let md5 = file.md5 || (await md5File(file.fullname));
                file.md5 = md5;
                file.uniqueId = await nanoid(5);
                processingPercent = Math.round(processingPercentAnimate + 20 * ((index + 1) / sourceFiles.length));
                sendEvent({
                    processingPercent,
        ...
        //source unique files
        uniqueFiles = sourceFiles.filter((f, i) => sourceFiles.findIndex((ff) => ff.md5 == f.md5) == i);
        /*
        ...
        if (!isDedupe) {
            targetFiles = await rraListWrapper(options.targetDirectory, {
                mode: rra.LIST,
                recursive: true,
                stats: true,
                ignoreFolders: true,
                ...
                if (!obj.isDirectory) {
                    let statItem = getStatItem(obj, "targetStats");
                    statItem.count++;
                    statItem.size += obj.stats.size;
                    sendEventStats({
            ...
            await sequential(targetFiles.map((file, index) => {
                return async () => {
                    let md5 = file.md5 || (await md5File(file.fullname));
                    file.md5 = md5;
                    file.uniqueId = await nanoid(5);
                    processingPercent = Math.round(processingPercentAnimate + 20 * ((index + 1) / targetFiles.length));
                    sendEvent({
                        processingPercent,
            ...
            existingFilesInTargetDir = uniqueFiles.filter((f) => targetFiles.findIndex((ff) => ff.md5 == f.md5) >= 0);
            ...
            consoleLog.info(`${existingFilesInTargetDir.length} files from source directory already in target directory`);
            targetFilesDupes = getDuplicatedFiles(targetFiles);
            targetFilesDupes.forEach((dupeFile) => {
        ...
        duplicatedFiles.forEach((dupeFile) => {
        ...
        uniqueFiles.forEach((file, index) => {
            let hasSomeWithSameName = uniqueFiles.some((f, i) => i != index && f.md5 != file.md5 && f.name == file.name && !f.newName);
        ...
        consoleLog.info(`${uniqueFiles.length} unique files detected (After checking duplicates in target directory)`);
        currCfg.set("sourceItems", sources.map((sourcePath) => {
            return {
                fullPath: sourcePath,
            };
        }));
    ...
    consoleLog.log(`${affectedFilesCount} files ${isDryRun ? "would be" : "will be"} ${mainActionLabelPast[options.mainAction.toString()]}`);
    if (isAnalysis) {
        ...
        if (["clean"].includes(options.mainAction)) {
            ...
            scope.resultStats.originalSize = sourceFiles.reduce((a, f) => a + f.stats.size, 0);
        ...
        if (["copy", "move"].includes(options.mainAction)) {
            ...
            scope.resultStats.originalSize = existingFilesInTargetDir.reduce((a, f) => a + f.stats.size, 0);
            let processingPercentAnimate = processingPercent;
            let copiedFiles = [];
            await sequential(uniqueFiles.map((file, index) => {
                return async () => {
                    ...
                    if (options.targetDirectoryStructure === "flat") {
                        ...
                        if (!isDryRun) {
                            await copyOrMoveFile(options.mainAction, sourceBasePath, sourceFileName, targetBasePath, targetFileName);
                        }
                    }
                    //DATE MODE
                    if (options.targetDirectoryStructure === "date") {
                        ...
                        targetBasePath = path.join(targetBasePath, createdDate.format("YYYY"), createdDate.format("MM"), createdDate.format("DD"));
                        targetPath = path.join(targetBasePath, targetFileName);
                        if (!isDryRun) {
                            await copyOrMoveFile(options.mainAction, sourceBasePath, sourceFileName, targetBasePath, targetFileName);
                        }
                    ...
                    if (!["flat", "date", "type"].includes(options.targetDirectoryStructure)) {
                        consoleLog.warn(`${options.targetDirectoryStructure} mode not implemented (Structure)`);
                    }
                    consoleLog.verbose(`${file.fullname} to ${targetPath} ${isDryRun
                        ? `(Would be ${mainActionLabelPast[options.mainAction].toLowerCase()})`
                        : `(${mainActionLabelPast[options.mainAction]})`}`);
                    processingPercent = Math.round(processingPercentAnimate +
            ...
            scope.resultStats.dedupeCount = copiedFiles.reduce((a, f) => {
                let dupesCount = duplicatedFiles.filter((ff) => {
            ...
            consoleLog.info(`${mainActionLabelVerb[options.mainAction]} process ended`);
        }
    }
    sendEvent({
        processing: false,
...
function getDuplicatedFiles(files) {
    return files.reduce((a, f) => {
        if (a.some((fff) => fff.md5 == f.md5)) {
            return a; //already processed
        }
        let dupesArr = files.filter((ff) => ff.md5 == f.md5 && ff.uniqueId !== f.uniqueId);
        a = [...a, ...dupesArr];
        return a;
    }, []);
}
async function removeFiles(files = [], options = {}) {
    let reason = options.reason || "(no-reason)";
    options.processingPercentAnimate = options.processingPercentAnimate;
    await Promise.all(files.map((f, index) => {
        return (async () => {
            if (!options.isDryRun) {
                await tryCatchAsync(() => sander.rimraf(f.fullname), {
                    path: f.fullname,
                }, "Remove operation fail");
                options.processingPercent = Math.round(options.processingPercentAnimate +
                ...
                consoleLog.verbose(`${f.fullname} (Removed from sources) (${reason})`);
            }
            else {
                consoleLog.verbose(`${f.fullname} (Would remove from sources) (${reason})`);
            }
        })();
    }));
    consoleLog.info(`${files.length} files ${options.isDryRun ? "should have been" : "were"} removed (${reason})`);
    return options.processingPercent;
}
function sumFilesSize(arr) {
    return arr.reduce((a, v) => {
        return a + v.stats.size;
    }, 0);
}
async function copyOrMoveFile(mainAction, sourceBasePath, sourceFileName, targetBasePath, targetFileName) {
    let targetPath = path.join(targetBasePath, targetFileName);
    if (mainAction === "copy") {
        await copyFile(path.join(sourceBasePath, sourceFileName), targetPath);
    }
    if (mainAction === "move") {
        await moveFile(sourceBasePath, sourceFileName, targetBasePath, targetFileName);
    }
}
async function copyFile(sourcePath, targetPath) {
    await tryCatchAsync(() => sander.copyFile(sourcePath).to(targetPath), {
        from: sourcePath,
        to: targetPath,
    }, "Copy operation fail");
}
async function moveFile(sourceBasePath, sourceFileName, targetBasePath, targetFileName) {
    await tryCatchAsync(() => sander
...
function sendEventStats(props) {
    win?.webContents.send("analysis_stat", props);
}
function sendEventStatsUsingCacheAnalysis(sourceFiles, targetFiles, duplicatedFiles) {
    scope.analysisStats.sourceStats = [];
    scope.analysisStats.targetStats = [];
...
async function rraListWrapper(path, options, callback = null) {
    callback = callback || (() => { });
    let res = [];
    ...
    if (!(res instanceof Array)) {
        ...
        handleErrorLogging(new Error("Readdir error"), {
...
function handleErrorLogging(err, textOrObjectOrFunction = "System error", text = "") {
    if (typeof textOrObjectOrFunction === "string" ||
        typeof textOrObjectOrFunction === "function") {
        consoleLog.error(typeof textOrObjectOrFunction === "string"
            ? textOrObjectOrFunction
            : textOrObjectOrFunction(err));
    }
    if (typeof textOrObjectOrFunction === "object") {
        consoleLog.error(`${text || "System error"}:${JSON.stringify({
            ...(textOrObjectOrFunction || {}),
            err: err.message,
        }, null, 4)}`);
    }
}
async function tryCatchAsync(cb, textOrObjectOrFunction = "System error", text = "") {
    try {
        await cb();
    }
    catch (err) {
...
function shouldSendConsoleEvent(level) {
    ...
    if (["error", "warn", "info", "verbose"].includes(level) &&
        logLevel === "verbose") {
        return true;
    }
    return false;
